...
final DynamicLibrary _lib = () {
  ...
  throw StateError(
    'Set SMOKE_BUILD_DIR to the CMake build directory '
    '(e.g. selection_hook_flutter/build/macos/)',
  );
}();
...
Future<void> main() async {
  ...
  final nativeCallable = NativeCallable<
    ...
    final msg = 'event: text="$text" program="$prog" method=${ref.method}';
    print(msg);
    eventCount.add(msg);
  });
  ...
  if (startResult != 0) {
    print('FAIL: sh_start returned $startResult');
    nativeCallable.close();
    sh_destroy(hook);
    exit(1);
  }
  print('[smoke] sh_start OK');

  print('[smoke] Waiting 5 seconds for events...');
  ...
  print('[smoke] Received ${eventCount.length} events');
  if (eventCount.length >= 4 && eventCount.length <= 5) {
    print('PASS: Got ${eventCount.length} events (expected 4-5)');
  } else {
    print('FAIL: Expected 4-5 events, got ${eventCount.length}');
    exit(1);
  }
