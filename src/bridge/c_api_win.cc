/**
 * Windows-specific bridge between the C-ABI API (c_api.h) and the Windows core
 * (SelectionHookCore). Pattern from c_api_mac.mm.
 */

#define _CRT_NONSTDC_NO_DEPRECATE
#include "c_api.h"
#include "../windows/selection_hook_core.h"
...
SH_API SHSelectionData *sh_get_current_selection(SelectionHook *hook)
{
    ...
    std::string utf8Text = StringPool::WideToUtf8(info->text);
    std::string utf8Program = StringPool::WideToUtf8(info->programName);

    result->text = strdup(utf8Text.c_str());
    result->program_name = strdup(utf8Program.c_str());
    ...
    result->pos_level = info->posLevel;
...
{
    if (!hook || !hook->core || !text)
        return SH_ERR_INVALID_ARG;
    std::wstring wideText = StringPool::Utf8ToWide(std::string(text));
    if (WriteClipboard(wideText))
        return SH_OK;
...
SH_API const char *sh_read_clipboard(SelectionHook *hook)
{
    ...
    if (!ReadClipboard(wideContent))
        return nullptr;
    if (wideContent.empty())
        return nullptr;
    hook->read_clipboard_buf = StringPool::WideToUtf8(wideContent);
